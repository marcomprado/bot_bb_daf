...
class GUIMain:
    ...
    def _executar_modo_individual(self):
        ...
        # Executa main() diretamente em thread separada
        def executar_main_direto():
            try:
                # Importa e executa main() diretamente
                from main import main as main_function
                codigo_saida = main_function()
                ...
                resultado = ResultadoProcesso(codigo_saida)
        ...
        # Inicia thread
        self.thread_execucao = threading.Thread(target=executar_main_direto, daemon=True)
    ...
    def _executar_modo_paralelo(self):
        ...
        # Executa funções main_parallel diretamente em threads
        def executar_funcoes_paralelas():
            try:
                from main_parallel import main as main_parallel_function
                arquivos_criados = resultado['arquivos_criados']
                
                # Lista para guardar threads e resultados
                threads_paralelas = []
                resultados = []
                
                def executar_instancia_paralela(arquivo_info):
                    # Configura sys.argv para simular linha de comando
                    argv_original = sys.argv.copy()
                    try:
                        sys.argv = ['main_parallel.py', arquivo_info['arquivo']]
                        codigo_saida = main_parallel_function()
                ...
                # Cria e inicia threads para cada instância
                for arquivo_info in arquivos_criados:
                    thread = threading.Thread(
                        target=executar_instancia_paralela, 
                        args=(arquivo_info,), 
                        daemon=True
                    )
                    threads_paralelas.append(thread)
                    thread.start()
                
                # Aguarda todas as threads terminarem
                for thread in threads_paralelas:
                    thread.join()
        ...
        self.thread_execucao = threading.Thread(target=executar_funcoes_paralelas, daemon=True)
        self.thread_execucao.start()
    ...
    def _cancelar_execucao(self):
        ...
        try:
            ...
            # As threads checam a flag _cancelado e terminam sozinhas
            # Não precisa terminar processos porque não usa subprocess
